/*
  ...
  This code is modified from the open-source repository of https://github.com/jameszah/ESP32-CAM-Video-Recorder-junior

  Just set a few parameters, compile and download, and it will record based on the remote control commands.
  Then pull out the sd and move it to your computer, and you will see all the recordings.
  ...
  by Huawei Wang April 09, 2022
   ...
   - v01 apr 09, 2022 - initial development
...
String TIMEZONE = "CET-1CEST-2,M3.5.0/02:00:00,M10.5.0/03:00:00";  // Amsterdam 
...
const char* ssid_rc = "H-Wang-H";
const char* password_rc = "";
int val;
...
void setup() {
  ...
  WiFi.begin(ssid_rc, password_rc);
...
void the_camera_loop (void* pvParameter) {
  ...
  while (1) {
    ...
    if (frame_cnt == 0 && start_record == 0) {
      ...
      // digitalWrite(33, frame_cnt % 2);                // blink, not light blink is needed
  ...
  if (client) {
    ...
    while (client.connected()) {
      ...
      while (client.available()>0) {
        int val = char(client.read()) - '0';
  
        if (val == 1) {
          client.write("Recording Started ..\n");
          start_record = 1;
          // digitalWrite(33, LOW);  // turn on the led flash light
        }
        else if (val == 0) {
          client.write("Recording Stopped ..\n");
          start_record = 0;  // stop recording  
          // digitalWrite(33, HIGH);  // turn on the led flash light
        }
        else {
